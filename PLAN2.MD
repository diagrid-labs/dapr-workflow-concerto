...
Build a minimal .NET web API that accepts Note records via POST, queues them in memory, and streams them to a single client via Server-Sent Events (SSE).
...
```bash
dotnet new web -n NoteStreamApi
cd NoteStreamApi
```
